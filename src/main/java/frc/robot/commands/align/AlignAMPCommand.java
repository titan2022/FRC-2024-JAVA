...
// public static final double TRANSLATIONAL_SPEED = 1;
    ...
    public Translation2d distanceFromAprilTag;
    public static final Rotation2d rotEpsilon = new Rotation2d(5 * DEG);
    public static final double transEpsilon = 0.01 * METERS;
    public boolean hasRotated;
    public boolean hasTranslated;

    public AlignAMPCommand(RotationalDrivebase rotationalDrive, TranslationalDrivebase translationalDrive, Localizer localizer) {
        this.rotational = rotationalDrive;
        this.translational = translationalDrive;
        this.localizer = localizer;
        addRequirements(rotationalDrive);
        addRequirements(translationalDrive);
    }
    ...
    public boolean isFinished() {
        if((Math.abs(aprilTagAngle.getRadians()) < rotEpsilon.getRadians())) {
            hasRotated = true;
            if(Math.abs(distanceFromAprilTag.getNorm()) < transEpsilon) {
                hasTranslated = true;
                return true;
            }
            return false;
