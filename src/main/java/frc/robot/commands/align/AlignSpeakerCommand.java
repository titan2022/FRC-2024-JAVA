...
public class AlignSpeakerCommand extends Command {
    ...
    public Translation2d speakerVector;
    ...
    public void execute() {
        if (Constants.getColor() == Alliance.Blue) {
            ...
            speakerVector = localizer.getRedSpeakerVector();
        } else {
            // distanceFromAprilTag = localizer.getTagPosition(RED_SPEAKER_APRILTAG);
            speakerVector = localizer.getBlueSpeakerVector();
        }
        Rotation2d speed = new Rotation2d(Math.copySign(ROTATIONAL_SPEED, speakerVector.getAngle().getRadians()));
    ...
    public boolean isFinished() {
        if (Math.abs(speakerVector.getAngle().getRadians() - localizer.getOrientation().getRadians()) < ANGLE_DEADBAND) 
            return true;
        else 
            return false;
    }
